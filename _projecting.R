...
cat("                              ","\n")
  ...
  projection_function<-function(input_dir,RCP,GCM,PERIOD,var_cho,m2,out_dir,m2_rep,models_to_ensemble,m2_eval,mInfo,evaluation){
    ...
    PER_dir<-paste0(RCP_dir,"/",PERIOD)
    GCM_dir<-paste0(PER_dir,"/",GCM)
    PERIOD_dir <- GCM_dir
...
###########################################################  
target_dir<-paste0(PERIOD_dir,"/","bio")
###########################################################      
    temp_layers <- c("bio_1","bio_2",paste0("bio_",5:11))
    prj_clim_layer<-lapply(paste0(target_dir,"/","/",paste0(colnames(m2@models$Sp$maxent[[1]]@object@presence),"")),raster)
# test <- raster(paste0(target_dir,"/","/","bio_4"))
# test_2 <- raster(paste0(current_clim_dir,"/","bio_4.tif"))
# max(test[],na.rm = T)
# max(test_2[],na.rm = T)


prj_clim_layer<-stack(prj_clim_layer);gc()
    for(i in 1:nlayers(prj_clim_layer)){
      
      if(any(names(prj_clim_layer[[i]]) %in% temp_layers)){
        prj_clim_layer[[i]] <- prj_clim_layer[[i]] /10
      } else {
        prj_clim_layer[[i]] <- prj_clim_layer[[i]]
      }
    }
...
p2m_all <- lapply(1:nrow(m2_eval),function(i){
    cat(i)
    x <- pred_rmaxent <- rmaxent::project(m2@models$Sp$maxent[[i]]@object, prj_clim_layer)
    x <- x$prediction_logistic#prediction_cloglog
    return(x)
  })
p2m_all <- stack(p2m_all)
names(p2m_all)<-paste0(mInfo$method,"_",mInfo$modelID)
writeRaster(p2m_all,paste0(PERIOD_proj_dir,"/",'proj_all.img'));gc()
p2m <- mean(p2m_all,na.rm=T)#(#calc(p2m_all, median)# median(p2m_all,na.rm=T)
writeRaster(p2m,paste0(PERIOD_proj_dir,"/",'proj.img'));gc() 
